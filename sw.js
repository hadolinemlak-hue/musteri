(() => {
  ...
  btnBackup?.addEventListener("click", async () => {
    ...
    const file = new File([text], "hadolin-crm-yedek.json", {type:"application/json"});
...
Notlar: ${item.notes || "-"}`;
  ...
  function scoreMatch(customer, portfolio){
    ...
    if(cb && pp){
      if(pp <= cb) score += 2;
      else if(pp <= Math.round(cb * 1.10)) score += 1;
      else score -= 1;
    }
...
İsterseniz size uygun olanları seçip detaylarını paylaşayım.`;
  ...
  todayListEl?.addEventListener("click", (e) => {
    ...
    if(cid){
      ...
      if(!item || !item.phone){ toast("Telefon yok."); return; }
    ...
    if(wid){
      ...
      if(!digits){ toast("Telefon formatı bozuk."); return; }
